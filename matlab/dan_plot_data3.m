...
%####################################################
function [] = dan_plot_data3(out_folder, data, avail, vartypes, schedulers)
...
[nof_vartypes, nof_schedulers, nof_flows, nof_time, nof_networks, nof_repetitions] = size(data);
    ...
    scale_t = [30 50 100 200 400 800 1600];
    scale_s = schedulers;%{'A\_false' 'Greedy' 'Optimal' 'Rnd', 'A\_true'};
     ...
     for n = 1:nof_networks
        ...
        for v=1:nof_vartypes
            ...
            if sum(avail_sq(:)) ==nof_repetitions*nof_time*nof_schedulers
                ...
                my_ylabel=labels{v};
                dan_tikz_out_errorbar(filename, data_sq, my_ylabel,legendlabels2,0, 1,0,0);
               ...
               % data_score(end,:,:) = 
                ...
                dan_tikz_out_errorbar(filename, data_score, my_ylabel,scale_s(2:end-1),1, 0,0,0);
            end
        end
