...
public class CostSeparation {
	...
	public int[][] statefulReward; 	//holding state of unused min_throughput reward; flows*timeslots matrix
	int[] flowReward;
	...
	private void setNetworkMatch() {
		int f=0;
		for(Flow flow:getTg().getFlows()){
			int n=0;
			System.out.print("Network Match Flow "+ f +": ");
			...
			for(Network net:ng.getNetworks()){
				...
				networkMatch[f][n] = 
						...
						cf.monetary(net);
				System.out.print(networkMatch[f][n]+", ");
...
//		//Printer.printInt("flowreward: ", flowReward);
	}
	
	public void updateStatefulReward(int f, int t, int tokens){
		Flow flow =getTg().getFlows().get(f); 
...
//		//////System.out.println("stateful_reward = ");//Arrays.deepToString(statefulReward));
//		for (int[] x : statefulReward) {
//			//Printer.printInt(x);
//			//////System.out.println("");
//		}
	}
	
	public double calcVio(int f, int n){
			// ////////System.out.println("f: " + f + " n: " + n);
			double c= getNetworkMatch(f, n) + getStatelessReward(f);
	...
	public void setTg(FlowGenerator tg) {
		this.tg = tg;
	}
