...
public class CostSeparation {
	...
	private void setNetworkMatch() {
		...
		for (Flow flow : tg.getFlows()) {
			...
			for (Network net : ng.getNetworks()) {
				networkMatch[f][n] = flow.getImpUser()
						* (CostFunction.latencyMatch(flow, net) + CostFunction.jitterMatch(flow, net))
						+ cf.monetary(net);
				n++;
			}
			f++;
	...
	public void updateStatefulReward(int f, int t, int tokens) {
		...
		for (int i = Math.max(0, t - flow.getWindowMin()); i <= Math.min(ng.getTimeslots() - 1,
				t + flow.getWindowMin()); i++) {
	...
	public int[] getNetworkMatch(int f) {
		int[] result = new int[ng.getNetworks().size()];

		for (int n = 0; n < ng.getNetworks().size(); n++) {
			result[n] = networkMatch[f][n];
		}
