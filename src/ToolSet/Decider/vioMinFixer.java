 ...
 *
 */
public class vioMinFixer extends GreedyScheduler implements Decider {
	protected String name;
	protected double fixThreshold;

	public vioMinFixer(NetworkGenerator ng, FlowGenerator fg, boolean NEW_RATING) {
		super(ng, fg);
	...
	public List<Decision> discoverDecisions(ScheduleWrapper sw) {
		...
		List<Integer> flows = sortFlowsByVioMin();
		...
		int[] vioTp = cf.vioTp(cf.cummulated_f_t(getTempSchedule()));
		for (int i = 0; i < flows.size(); i++) {
			if ((double) vioTp[flows.get(i)] / totalViolation > fixThreshold || true) {
	...
	protected List<Integer> sortFlowsByVioMin() {
