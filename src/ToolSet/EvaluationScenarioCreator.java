...
public class EvaluationScenarioCreator {
	 ...
	 */
	public static Vector<Scheduler> initSchedulers(NetworkGenerator ng, FlowGenerator tg, int[][][] schedule) {
		...
		schedulers.add(new MLPrio(ng, tg, 0.0));
	...
	private void writeScenarioLog(int evaluateMax) {
		...
		logger.logSchedulers(initSchedulers(null, null, null)); //save which schedulers are available for evaluation
	...
	public void calculateInstance_t_n_i(int t, int n, int f, int rep, String folder, boolean overwrite, boolean recalc,
			boolean decomposition_heuristic) {
		int time = 25 * pow(2, t);
		int nets = pow(2, n);
		int flows = pow(2, f);
		...
		EvaluationScenarioExecutionWorker worker = new EvaluationScenarioExecutionWorker(TEST_COST_FUNCTION, VISUALIZE,
				time, nets, flows, rep, folder_out, overwrite, recalc);
		taskList.add(worker);
