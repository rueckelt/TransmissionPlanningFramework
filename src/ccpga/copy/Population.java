...
public class Population {
     ...
     */
    // Create a population
    public Population(double amp, boolean initialise) {
    	this.setAmp(amp);
        ...
        if (initialise) {
            ...
            //////////System.out.println("pop-i: " + i + " *****************************");
                ...
                // get the netnumber
                Random r = new Random();
                int genePoolSize = Individual.getGenePool().size();
                int x = r.nextInt(genePoolSize * 2);
                int net = 0;
                if (genePoolSize != 0) {
                    net = x > genePoolSize? 0: Individual.getCertainGene(i % Individual.getGenePool().size());
    ...
    // Get population size
    public int size() {
        return Combination.getActiveFlowNum();//getIndividuals().length;
    }
