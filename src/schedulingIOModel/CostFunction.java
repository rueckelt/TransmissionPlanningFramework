...
public class CostFunction {
	...
	public static int vioTimeLimits(Flow flow, int t) {
		return (int) (Math.pow(Math.max(t - flow.getDeadline(), 0), 2) * flow.getImpDeadline()
	...
	public int vioTp_weight(int vioTpMin, Flow flow) {
		...
		return vioTpMin * flow.getImpThroughputMin() * (int) (minTpAmplifier * 1.02)
				/ (flow.getTokensMin() * flow.getWindowMin());
	}
	...
	public int costViolation(int[][][] schedule) {
		...
		for (int f = 0; f < flows; f++) {
			...
			cost_vio += (vioSt(schedule)[f] + vioDl(schedule)[f] + vioNon(cummulated_f_t)[f] + vioTp(cummulated_f_t)[f] //impUser is squared for vioNon
					+ vioLcy(schedule)[f] + vioJit(schedule)[f]) * flow.getImpUser();
		}
