...
public class Network implements Serializable, Cloneable{
	...
	public String toString(){
		...
		s += "]\n";
		//s += "Jitter: " + getJitter() + "\n";
		//s += "Latency: " + getLatency() + "\n";
		return s;
	}
	public String toString(boolean hello){
		String s = "[";
		boolean first = true;
		for (Integer slot : capacity) {
			if(first){
	...
	public static Network getWiFi(int slots, int meanChunks, int delay, boolean random){
		Network wifi = new Network(slots*1/2, meanChunks);
		wifi.addRampUp(slots/4);
		wifi.addRampDown(slots/4);
		wifi.delay(delay);
	...
	public static Network getCellular(int slots, int meanChunks, boolean random){
		Network cell = new Network(slots, meanChunks);
		if (random) {
			cell.setType(2);
			cell.setCost(RndInt.get(15, 25));
	...
	public void addPositionUncertainty(Vector<Integer> slotIndexes) {
		...
		capacity = (Vector<Integer>) cap_tmp.clone();
	 ...
	 * @param w_char
	 */
	public void addCharacteristicsUncertainty(float w_char) {
		//throughput
		int filtered=0;
...
//				System.out.print(filtered+", ");
	...
	public void addRangeUncertainty(float w_range){
