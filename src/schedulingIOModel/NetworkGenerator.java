...
public class NetworkGenerator implements Serializable, Cloneable {
	...
	private int[] interfacesOftype = {1,1,1};	//default: (index: 0 = NONE, 1 = #wifi, 2 = #mobile network) 
...
//	} 
	...
	public void log(String path, String content) {
		new File(path).mkdir(); // + uncertainty + "/").mkdir();
		File log = new File("1.txt");
	    try{
		    if(log.exists() == false){
	...
	public Vector<Integer> diff(Vector<Integer> orig, Vector<Integer> changed) {
		Vector<Integer> diff = new Vector<Integer>();
		for (int i = 0; i < orig.size(); i++) {
			diff.add(changed.get(i) - orig.get(i));
		}
		return diff;
	}
