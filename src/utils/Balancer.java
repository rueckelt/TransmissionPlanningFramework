...
//		}
		...
		double rating = config.getCs().calcVio(f, n)+	//stateless reward + network match
	...
	/**
	 *  calc the ratingof each flow in this network
...
//		int monetaryCost = 0;
		...
		Flow flow = config.getFg().getFlows().get(f);
		double rating = config.getCs().calcVio(f, n)+	
