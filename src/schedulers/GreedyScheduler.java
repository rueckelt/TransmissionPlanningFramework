...
public class GreedyScheduler extends Scheduler {
	...
	public String getType() {
		String s = new String("Greedy_" + schedule_decision_limit + (NEW_RATING_ESTIMATOR ? "_H2" : "")).replace("-",
				"m");
		return s;
	}
	...
	protected boolean scheduleDecision(int f, int n, int t) {
		...
		if (NEW_RATING_ESTIMATOR) {
			int rating = calcVio(f, n) + //stateless reward + network match
					cs.getStatefulReward(f, t) + cs.getTimeMatch(f, t) / getAvMinTp(tg.getFlows().get(f)); //time limit violation is only counted for used slot. not for token amount
	...
	protected int calcVio(int f, int n) {
		if (NEW_RATING_ESTIMATOR) {
			...
			int c = ((CostFunction.jitterMatch(flow, network) //match functions return 0 for match, else strength of violation
					...
					- throughputMatch(flow, network) - flow.getImpUnsched() * flow.getImpUser() //each unscheduled chunk leads to this cost
			) * flow.getImpUser() + network.getCost() * ng.getCostImportance(); //cost independent from flow user weight
	...
	/**
	 * Mainly used by the GreedyScheduleFNGT Algorithm
	 */
